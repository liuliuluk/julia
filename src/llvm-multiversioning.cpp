...
private:
    ...
    void expand_clone(ValueToValueMapTy &vmap, const std::string &suffix);
    ...
    std::vector<GlobalVariable*> gvars;
    Module &M;
...
CloneCtx::CloneCtx(MultiVersioning *pass, Module &M)
    : ctx(M.getContext()),
      ...
      pass(pass),
      specs(jl_get_llvm_clone_targets(JL_LLVM_VERSION)),
      fvars(consume_gv<Function>(M, "jl_sysimg_fvars")),
      gvars(consume_gv<GlobalVariable>(M, "jl_sysimg_gvars")),
      M(M)
{
    groups.emplace_back(0, specs[0]);
    size_t ntargets = specs.size();
    ...
    size_t nfvars = fvars.size();
    for (size_t i = 0; i < nfvars; i++)
...
void CloneCtx::clone_bases()
{
    if (!has_cloneall)
        return;
    size_t ngrps = groups.size();
    for (size_t gid = 1; gid < ngrps; gid++) {
        auto &grp = groups[gid];
...
{
    bool has_loop = !pass->getAnalysis<LoopInfoWrapperPass>(F).getLoopInfo().empty();
    bool has_vec = is_vector(F.getFunctionType());
    if (has_vec)
        has_veccall = true;
    bool has_math = false;
    auto done = [&] () {
        return has_vec && has_math && has_veccall;
    };
    for (auto &bb: F) {
        for (auto &I: bb) {
...
void CloneCtx::expand_clone(ValueToValueMapTy &vmap, const std::string &suffix)
{
...
{
    assert(!vars.empty());
    add_comdat(GlobalAlias::create(GlobalVariable::ExternalLinkage, name + "_base", vars[0]));
            ...
            if (i == 0 || spec.flags & JL_TARGET_CLONE_ALL) {
                ...
                for (uint32_t j = 0; j < nfvars; j++) {
                    ...
                    if (shared_relocs.count(j)) {
                        ...
                        count++;
                        idxs.push_back(tgt->relocs.count(j) ? (tag_mask | j) : j);
                        offsets.push_back(get_ptrdiff32(cast<Function>(f), fbase));
                    }
                }
            }
...
bool MultiVersioning::runOnModule(Module &M)
{
    ...
    // Replace them with `undef` slot to be initialized at runtime and record relocation slot.
